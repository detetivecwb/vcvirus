...
async function handleVerifySchedules(job) {
  try {
    const { count, rows: schedules } = await Schedule.findAndCountAll({
      ...
      include: [{ model: Contact, as: "contact" }, { model: Whatsapp, as: "whatsapp" }]
    });
    if (count > 0) {
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    console.log(schedule.whatsappId)
    let whatsapp = await Whatsapp.findByPk(schedule.whatsappId);

    if (!whatsapp)
      whatsapp = await GetDefaultWhatsApp(schedule.companyId);

    if (whatsapp.status !== "CONNECTED") {
      logger.info(`Whatsapp não está ativo: ${whatsapp.name}`);
      return;
    }
        ...
        c.queues.map(async q => {
          ...
          if (count > 0) {
            for (const ticket of tickets) {
              ...
              let sendGreetingMessageOneQueues = false;
              ...
              settings.forEach(setting => {
                if (setting.key === "sendGreetingMessageOneQueues") {
                  sendGreetingMessageOneQueues = setting.value === "enabled";
                }
              });
              ...
              if (!userId) {
                ...
                if (await findUserById(randomUserId, ticket.companyId) > 0) {
