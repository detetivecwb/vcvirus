  ...
  const query = `
    ...
    traking as (
      select
        ...
        (coalesce(tt."closedAt",tt."finishedAt") is not null) "finished",
        (tt."userId" is null and coalesce(tt."closedAt",tt."finishedAt") is null) "pending",
        coalesce((
          (date_part('day', age(coalesce(tt."ratingAt", coalesce(tt."closedAt",tt."finishedAt")) , tt."startedAt")) * 24 * 60) +
          (date_part('hour', age(coalesce(tt."ratingAt", coalesce(tt."closedAt",tt."finishedAt")), tt."startedAt")) * 60) +
          (date_part('minutes', age(coalesce(tt."ratingAt", coalesce(tt."closedAt",tt."finishedAt")), tt."startedAt")))
        ), 0) "supportTime",
        coalesce((
    ...
    counters as (
      select
        ...
        (select count(id) from "Tickets" t where "status" = 'closed' and t."companyId" = ?) "tickets",
        (select count(id) from "Tickets" t where t."status" = 'nps' and t."companyId" = ?) "waitRating",
