...
export const index = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { whatsappId }: WhatsappData = req.body;
  const { msdelay }: any = req.body;
  const { body, quotedMsg, userId, queueId }: MessageData = req.body;
  ...
  if (medias) {
    await Promise.all(
      medias.map(async (media: Express.Multer.File) => {
        await SendWhatsAppMedia({ body, media, ticket: contactAndTicket, isForwarded: false });
      })
    );
  } else {
    await SendWhatsAppMessageAPI({ body, whatsappId: whatsapp.id, contact: contactAndTicket.contact, quotedMsg, msdelay });
  }
